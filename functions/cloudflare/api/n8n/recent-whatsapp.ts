...
/**
 * GET /api/n8n/recent-whatsapp?limit=5
 * Fetches recent WhatsApp messages from n8n webhook
 * 
 * curl "http://localhost:8788/api/n8n/recent-whatsapp?limit=5"
 */
export const onRequestGet: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    // Call n8n webhook to get recent WhatsApp messages
    const n8nUrl = `${context.env.N8N_BASE_URL}/webhook/recent-whatsapp?limit=${limit}`
    ...
    if (!response.ok) {
      console.error('n8n WhatsApp fetch failed:', response.status)
      return jsonResponse({
        ok: true,
    ...
    const items = (data.messages || data.items || []).slice(0, parseInt(limit)).map((msg: any) => ({
    ...
    console.error('Recent WhatsApp error:', error)
